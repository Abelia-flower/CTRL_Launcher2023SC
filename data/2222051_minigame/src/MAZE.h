...
public:
	...
	int cellSize = 20;
	int offsetX = (Scene::Width() - mazeWidth * cellSize) / 2;
	int offsetY = (Scene::Height() - mazeHeight * cellSize) / 2;
	...
	Array < Array<int>> maze = generateMaze(mazeWidth, mazeHeight);	//迷路生成
...
public:
	...
	void draw() const override
	{
		...
		text3(U"エンターキーを押してタイトルに戻る").draw(95, 510, Palette::Blue);
	}
};
