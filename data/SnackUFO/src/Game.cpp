...
void snk::GameSceneParamInit()
{
	...
	set_param(1, U"難易度調整係数", U"difCoe");
	set_param(13, U"敵のhp", U"enemyHp");

	set_nest(U"Barrages", U"shootingStar");
	set_param(1, U"難易度調整係数", U"difCoe");
	set_param(15, U"敵のhp", U"enemyHp");
...
void Game::Init()
{
	...
	field_hitbox->draw_hitbox_outline = true;
	...
	//ポーズ画面を構成
	{
		...
		//はい、いいえのフェードインフェードアウト
		{
			...
			std::shared_ptr<int32> selecting(new int32);//yesNoの選択
			//終了したらtrue
			yesOrNoAction->add(new prg::FuncAction(
	...
	//ゲーム画面外の構成 カメラに追従するようにしている
	{
		...
		sousa->z = 100;
		sousa->transform->setPos(Vec2{ Scene::Size() } - Vec2{ 310,70 });
	...
	auto setEnemyHp = [=](double maxHp) {
		*enemyMaxHp = maxHp;
		mainEnemy->GetComponent<ActorState>()->hp = maxHp;
	};
	//銃を消す　弾は消さない
	...
	//自機狙い
	{
		...
		barrageAction->add(new prg::FuncAction(
			...
			[=]
			{
				behavior->progress->stopAllAction({U"yureru"});
				eraseEnemyAllGuns();
			},
			Math::Inf));
		gameProgress->add(barrageAction);
	...
	//大量のUFOが流れてくる
	{
		...
		sentakuki->setStartCondition(new prg::FuncCondition([=] {return mainEnemy->GetComponent<ActorState>()->hp <= maxHp / 2; }));
	...
	//クリア演出
	{
		...
		gameProgress->add(new prg::FuncAction([=] {
				...
				drawing->drawingFunction = [=](const DrawFunc*)
				{
					...
					Rect{ 0,0,Scene::Size() }.draw(*color);
				};
	...
	//背景
	{
		...
		{
			RectF{ mainFieldPos,mainFieldW*1.5,mainFieldH*1.5 }.draw(Arg::top = Color{ 50,70,220 }, Arg::bottom = Palette::Skyblue);
		};
		backGround->z = -Math::Inf;
