    ...
    def move(self): 
        ...
        if self.first:    #最初にShiftを押されるまで
            ...
            if taegets_collided and score.score < 20 and left_time > 0:
                ...
                after = After(resourcePath("resources\gazo\school_tani_otosu_boy.png"),taegets_collided[0].rect.x,taegets_collided[0].rect.y,100)
                target = Target(resourcePath("resources\gazo\school_tani_get_boy.png"),rd.randint(1,SCREEN.width-80),rd.randint(1,SCREEN.height-80),my_randint(-velo,velo,-low,low),my_randint(-velo,velo,-low,low),80,False)
                score.add_score(touch)
                pygame.mixer.music.load(resourcePath("resources\music\デデドン.mp3"))
...
class Strings:   #文を表示するクラス
    def __init__(self,size,filename):
        self.bigfont = pygame.font.Font(resourcePath("resources\shippori3\ShipporiMincho-Medium.otf"),int(size*2.5))
        self.titlefont = pygame.font.Font(resourcePath("resources\shippori3\ShipporiMincho-ExtraBold.otf"),int(size*1.6))
...
def main():
    ...
    font = pygame.font.Font(resourcePath("resources\shippori3\ShipporiMincho-ExtraBold.otf"),40)
    big_font = pygame.font.Font(resourcePath("resources\shippori3\ShipporiMincho-ExtraBold.otf"),100)

    strings = Strings(50,resourcePath("resources\shippori3\ShipporiMincho-Bold.otf"))
    title_image = pygame.image.load(resourcePath("resources\gazo\TITLE.jpg")).convert_alpha()
    illust_image = pygame.image.load(resourcePath("resources\gazo\ILLUST.jpg")).convert_alpha()
    difficulty_image = pygame.image.load(resourcePath("resources\gazo\DIFFICULTY.jpg")).convert_alpha()

    pygame.mixer.init(frequency = 44100)   #音楽の再生
    death_BGM = pygame.mixer.Sound(resourcePath("resources\music\8bitBGM.mp3"))
    death_BGM.set_volume(0.03)
    TITLE = pygame.mixer.Sound(resourcePath("resources\music\8bitTown.mp3"))
    TITLE.set_volume(0.03)
    PLAYING = pygame.mixer.Sound(resourcePath("resources\music\Endurance_Game.mp3"))
    PLAYING.set_volume(0.03)
    celemony = pygame.mixer.Sound(resourcePath("resources\music\楽しく面白く.mp3"))
    celemony.set_volume(0.03)
    clear = pygame.mixer.Sound(resourcePath("resources\music\きこりの森.mp3"))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:   #難易度選択
                ...
                if type(difficulty) == str:   #難易度が選ばれたら...
                    ...
                    running = True
                    for i in range(num):
                        target = Target(resourcePath("resources\gazo\school_tani_get_boy.png"),rd.randint(1,SCREEN.width-80),rd.randint(1,SCREEN.height-80),my_randint(-velo,velo,-low,low),my_randint(-velo,velo,-low,low),80,True)  #ターゲットの生成
                        targets.append(target)
                        targets_rect.append(target.rect)
                        TITLE.stop()
                    player = Player(resourcePath("resources\gazo\Text_kadai2.png"),200,200,v_play,v_play,80,Targets,screen) #課題の生成
                    difficult = False
            if event.type == KEYDOWN:
                if event.key == K_ESCAPE:
                    pygame.quit()
                ...
                if event.key == K_m:   #もしMが押されたら(隠し要素)
                    ...
                    running = True
                    for i in range(20):
                        target = Target(resourcePath("resources\gazo\school_tani_get_boy.png"),rd.randint(1,SCREEN.width-80),rd.randint(1,SCREEN.height-80),my_randint(-velo,velo,-low,low),my_randint(-velo,velo,-low,low),80,True)
                        targets.append(target)
                        targets_rect.append(target.rect)
                        TITLE.stop()
                    player = Player(resourcePath("resources\gazo\Text_kadai2.png"),200,200,v_play,v_play,80,Targets,screen) #課題の生成
